...
public class CreationReport extends AbstractReport {
    ...
    private long getOpenIssueCount(ApplicationUser user, Date startDate, Date endDate, Long projectId) throws SearchException {
        ...
        Query query = queryBuilder.where().createdBetween(startDate, startDate).and().project(projectId).buildQuery();
       //  Query query = queryBuilder.where().created(startDate).and().project(projectId).buildQuery();
    ...
    public void validate(ProjectActionSupport action, Map params) {
        try {
            startDate = formatter.parse(ParameterUtils.getStringParam(params, "dueDate"));
        } catch (IllegalArgumentException e) {
            action.addError("dueDate", action.getText("report.issuecreation.startdate.required"));
